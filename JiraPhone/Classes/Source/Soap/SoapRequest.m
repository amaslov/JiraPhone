...
//	NSLog(@"\nXML RESPONSE:\n %@ \n", [doc description]);
	...
	if([fault hasFault]) {
		...
		CXMLNode* bodyElement = [Soap getNode: [doc rootElement] withName: @"Body"];
		if(deserializeTo == nil) {
			output = [Soap deserialize:bodyElement];
		} else {
			if([deserializeTo respondsToSelector: @selector(initWithNode:)]) {
				output = [deserializeTo initWithNode: bodyElement];
			} else {
				NSString* value = [[[bodyElement childAtIndex:0] childAtIndex:0] stringValue];
